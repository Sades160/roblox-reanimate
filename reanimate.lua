...
if Humanoid.RigType == Enum.HumanoidRigType.R15 then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "[SadesReanimate] You are on R15! Please use the R15 Reanimation instead.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
return end
if game.Players.LocalPlayer.Character:FindFirstChild("SadesReanimate") or workspace:FindFirstChild("SadesReanimate") then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "[SadesReanimate] You are already reanimated! Please reset to unreanimate.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
...
game:WaitForChild("Run Service").Heartbeat:Connect(function()
    if Reanimated == true then
        if Character.SadesReanimate then
            Character.Torso.CFrame=Character.SadesReanimate.Torso.CFrame
        end
...
CopyCharacter.Name = "SadesReanimate" -- you can always change name.

for k,l in pairs(Character.SadesReanimate:GetDescendants()) do 
...
game:GetService("UserInputService").JumpRequest:connect(function(t)
    if CopyCharacter.Humanoid.FloorMaterial~=Enum.Material.Air then 
        ...
        game.Players.LocalPlayer.Character.SadesReanimate:FindFirstChildOfClass('Humanoid').Sit=false 
    end 
end)
